...
async function deployTemplateInSandbox(config: DeploymentConfig) {
  ...
  try {
    ...
    // Create template from E2B template ID
    ...
    const initResult = await sandbox.commands.run(
      `echo "Template ${templateId} is being used for this sandbox"`,
      {
...
async function main() {
  ...
  try {
    const deployment = await deployTemplateInSandbox({
      templateId: '4tubnivta0k3mir0t3yd',
      templateName: 'hack-skeleton-joke',
      envVars: {
    ...
    // Share URL with user
    console.log('📤 Share this URL with your users:')
